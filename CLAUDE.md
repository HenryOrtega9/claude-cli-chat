...
8. A background agent that finishes while the parent turn is still running produces NO `<task-notification>` user turn: the CLI folds the XML into the model's context as a `queued_command` attachment. The only stdout signal is `{"type":"system","subtype":"task_notification","tool_use_id","status","summary"}` (verified on CLI 2.1.280). Handling only the XML path leaves the card and the "N agents running…" pill stuck after the chat ends. Both paths go through `TabController.settleBackgroundAgent`. The same event also fires for background Bash tasks, so it filters on Task/Agent.
9. A user message written to stdin while a turn runs is queued by the CLI and handed to the model at the next tool boundary, inside the same turn (one `result`). If it lands during the final reply it runs as a fresh turn right after (second `system/init` + `result`). The only signal of WHEN it was delivered is the `--replay-user-messages` ack: a `user` event with `isReplay: true` and the `uuid` we sent (verified on CLI 2.1.283). Steering (`TabController.queuedSteers`) keys on that ack; `handleResult` holds busy while anything is still queued. The iOS gateway already spawns with the same flag, so replay echoes reach every client; `handleUserEcho` drops any whose uuid isn't one of its own queued steers. A queued message can be taken back with the control request `{type:"control_request",request_id,request:{subtype:"cancel_async_message",message_uuid}}`; the CLI answers with a `control_response` whose payload is `{cancelled: boolean}` (false once it was already handed to the model). `TabSession.cancelQueuedMessage` consumes that response; `TabController.retractQueuedSteer` handles the race.
