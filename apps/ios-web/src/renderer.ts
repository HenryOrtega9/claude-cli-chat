...
import { setHardwareKeyboardPresent } from "../../../src/view/InputBox";
  ...
  installHandler((name, payload) => {
    switch (name) {
      ...
      case "hardwareKeyboard":
        /* NativeBridge's GCKeyboard observation: flips the composer between
           software-keyboard Enter (newline, send button submits) and
           physical-keyboard Enter (submits, Shift+Enter newlines). */
        setHardwareKeyboardPresent(payload.present === true);
        return;
      case "share": {
