...
The web UI itself is built elsewhere (`npm run build:ios` → `apps/ios/Web/`). This
...
```sh
cp apps/ios/Sources/Secrets.example.swift apps/ios/Sources/Secrets.swift   # once
npm run build:ios                                                # fills apps/ios/Web/
...
`apps/ios/Web/` must exist before generating: it is a folder reference (blue folder),
...
```sh
# VAULT_GATEWAY_BIND=127.0.0.1 (see daemons/gateway/src/config.ts) keeps the
...
picker). `apps/ios/ShareExtension/ShareViewController.swift` is the entire
...
`apps/ios/Sources/ShareInbox.swift` (main-app target) is the reader.
...
- **Cold launch losing the dispatch, not just for Share** (this predated this
  ...
  an empty `#app`. Fixed by `IosChatShell.handleShare` (`apps/ios-web/src/shell.ts`),
...
- `apps/ios-web/src/renderer.ts`'s `case "share"` now parses `payload.images` (a
  defensive `parseSharePayload`, dropping any malformed entry) alongside
  `payload.text` and calls `shell.handleShare({text, images})`.
- `IosChatShell.handleShare` / `applyShare` (`apps/ios-web/src/shell.ts`) insert the
...
**Verified** (`apps/ios/build`, simulator `iPhone 17 Pro`, `xcodebuild -scheme
...
- **Draft-preservation through a resync, confirmed** (a separate fix this same
  pass — `IosChatShell.resyncTab`, `apps/ios-web/src/shell.ts`): `resyncTab` used
