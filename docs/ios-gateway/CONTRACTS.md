...
- ~~Known open bug (wave 1, unfixed): a tab created by `POST /tabs` but never given a turn before the daemon restarts…~~ **Fixed (integration).** `src/engine.ts` no longer infers an established session from the fact of rehydration. `canResume()` answers from evidence: this process saw the child's `system/init`, or the CLI wrote a transcript at `~/.claude/projects/<slug>/<sessionId>.jsonl` (`sessionFilePathFor`, now in the node-free `src/claude/session-files.ts`). Neither ⇒ `--session-id`. Belt and braces: a `--resume` child that dies before `system/init` is treated as a rejected session id: the tab respawns once with `--session-id` and re-sends the same blocks, since nothing streamed. `scripts/gateway/test/smoke.mjs` section 7 is the regression (create a tab, `launchctl kickstart -k`, turn, expect `success`); it skips itself when the launchd job is not loaded or `--no-restart` is passed.
